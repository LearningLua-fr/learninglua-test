function contains_function_definition(code)
    return string.match(code, "function%s+ReverseWordsClean%s*%(%s*[%w_]+%s*%)") ~= nil
end

function function_is_called(code)
    return string.match(code, "ReverseWordsClean%s*%(%s*['\"]") ~= nil
end
...
function contains_hardcoded_output(code)
    return string.match(code, 'print%s*%(%s*["\']?awesome is Lua world Hello["\']?%s*%)') ~= nil
end

function output_is_correct(user_output, expected_output_user)
    return user_output:match("^%s*awesome is Lua world Hello%s*$") ~= nil
end
...
function run_test(user_code, user_output, expected_output_user)
    ...
    if contains_function_definition(user_code) then
        print("Test 1/5 Passed: Function 'ReverseWordsClean' is defined")
        passed = passed + 1
    else
        print("Test 1/5 Failed: Function 'ReverseWordsClean' is missing")
    end

    if function_is_called(user_code) then
        print("Test 2/5 Passed: Function is called")
        passed = passed + 1
    else
        print("Test 2/5 Failed: Function is not called")
    end
    ...
    if #output_lines == 1 and output_is_correct(output_lines[1], expected_output_user) then
